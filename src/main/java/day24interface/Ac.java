...
public interface Ac {
    ...
    //3)"default" in "bacteriaKiller()  method" is not "access modifier", it is a new keyword to create "method with body"
   // in an interface
    ...
    //If you need a "method with body" and access ot it just by using "interface name" use "static"keyword.
    static void climate(){
    ...
    //Variables are "public" in an interface as default, that is why when you create a variable  no need to declare
   // it as "public"
    //Variables are "static" in an interface as default, that is why when you try to access to a variable no need to
    // create an object use just interface name

    int price=2000;

    //Note : An interface may have "public" or"default" access modifier, you cannot use "protected" and "private"
    //access modifiers sam in "Classes".
